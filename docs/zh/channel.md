...
- **本项目实现**：`scripts/vector_fitting_py.py` - 基于 `vectfit3.m`（版本 1.0, 2008）的完整 Python 翻译
...
向量拟合是一种迭代优化算法，最初由 Bjørn Gustavsen 和 Adam Semlyen 提出 [1]，将频域响应S(f)近似为有理函数形式。本项目的 Python 实现 `scripts/vector_fitting_py.py` 基于 [SINTEF VFIT3](https://www.sintef.no/en/software/vector-fitting/downloads/vfit3/) 的 `vectfit3.m`（版本 1.0, 2008）进行完整翻译，实现了 Fast Relaxed Vector Fitting 算法。
